...
def _extract_date_code(donor_folder_name: str) -> Optional[str]:
    match = re.search(r"(\d{1,2})[/-](\d{1,2})[/-](\d{2,4})", donor_folder_name)
    if not match:
        return None
...
def _extract_donor_id(donor_folder_name: str) -> Optional[str]:
    numbers = re.findall(r"\d+", donor_folder_name)
    return numbers[-1] if numbers else None


def build_donor_context(base_path: Path) -> Dict[str, Optional[str]]:
    donor_folder = base_path.name
    response_folder = base_path.parent.name

    return {
        "response": _normalize_response_label(response_folder),
        "donor_id": _extract_donor_id(donor_folder),
        "date": _extract_date_code(donor_folder),
    }
...
def extract_donor_id_from_path(base_path: Path) -> Optional[str]:
    return _extract_donor_id(base_path.name)


def format_donor_response_label(response: str) -> str:
...
def format_donor_date_label(donor_folder_name: str) -> Optional[str]:
    match = re.search(r"(\d{1,2})[/-](\d{1,2})[/-](\d{2,4})", donor_folder_name)
    if not match:
        return None
...
def processed_channel_filename(filename: str) -> str:
    """Return the preprocessed channel filename produced by ImageJ preprocessing."""
...
def build_channel_filename(
    ...
    parts = [
        format_donor_response_label(response),
        format_donor_date_label(donor_folder_name) or "UnknownDate",
        "DLBCL",
