...
def normalize_image_filter_config(config):
    """
    Normalize image filter input into (per-sample map, default list).
...
def selected_sample_numbers(samples_to_process):
    """Normalize sample filter input to a set of sample numbers."""
    if not samples_to_process:
        return None

    values = samples_to_process.keys() if isinstance(samples_to_process, dict) else samples_to_process
    selected = set()
    for value in values:
        try:
