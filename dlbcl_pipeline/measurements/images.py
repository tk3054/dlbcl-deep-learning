...
from dlbcl_pipeline.utils.config_helpers import filter_image_folders, normalize_image_filter_config
...
def combine_sample(sample_name, base_path, images_to_process=None, verbose=True):
    """
    ...
    Args:
        ...
        images_to_process: Optional image filter for this sample or all samples
    ...
    donor_id = extract_donor_id_from_path(Path(base_path)) or "UnknownDonor"
    sample_df['unique_id'] = sample_df.apply(
        lambda row: f"{donor_id}_{_sample_number(row['sample'])}_{_image_number(row['image'])}_{int(row['cell_id']):02d}",
        axis=1,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base-path", default=BASE_PATH, required=BASE_PATH is None, help="Base directory path")

    args = parser.parse_args()

    if not args.base_path:
        print("Error: base path not set. Provide --base-path.")
        sys.exit(1)
