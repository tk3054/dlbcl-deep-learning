"""ImageJ runtime setup and related ImageJ run helpers."""
...
from dlbcl_pipeline.utils.config_helpers import extract_sample_number, selected_sample_numbers
from dlbcl_pipeline.utils.folder_resolution import resolve_channel_filenames, resolve_image_folder
from dlbcl_pipeline.utils.name_builder import (
    NameBuilder,
    build_donor_context,
...
def initialize_imagej(mode: str = "headless", verbose: bool = True):
    """Initialize ImageJ once for a pipeline run."""
    if verbose:
        print(f"Initializing ImageJ in {mode} mode (will be reused for all images)...")
    import imagej
    with suppress_output():
        ij = imagej.init("sc.fiji:fiji", mode=mode, add_legacy=True)
    if verbose:
...
def prepare_run(base_path: str, samples_to_process: list[int], ij_instance=None):
    ...
    if selected_samples:
        sample_folders = [
            s for s in sample_folders if extract_sample_number(s) in selected_samples
        ]
    ...
    ij = initialize_imagej(verbose=True)
...
def prompt_channel_filenames(
    ...
    """
    Require configured channel filenames to exist for one image folder.
    """
    image_dir = base_path_obj / sample_folder / image_number

    return resolve_channel_filenames(
        image_dir=image_dir,
...
def export_images_for_export(
    ...
    context = build_donor_context(base_path_obj)
    response = context.get("response") or "unknownresponse"
    donor_id = context.get("donor_id") or "UnknownDonor"
