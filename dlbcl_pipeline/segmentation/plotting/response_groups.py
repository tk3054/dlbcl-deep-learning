...
Creates a grid comparing all donors with color-coded backgrounds.
...
def plot_all_comparison(base_dir, verbose=True):
    """
    Create comparison grid for all donors (responders + non-responders).
    ...
    # Load both CSVs
    responder_csv = base_dir / "responder" / "all_donors_combined.csv"
    non_responder_csv = base_dir / "non-responder" / "all_donors_combined.csv"
    ...
    if responder_csv.exists():
        ...
        dfs.append(df_r)
        if verbose:
            print(f"Responders: {df_r['donor_id'].nunique()} donors, {len(df_r)} cells")
    ...
    if non_responder_csv.exists():
        ...
        dfs.append(df_nr)
        if verbose:
            print(f"Non-responders: {df_nr['donor_id'].nunique()} donors, {len(df_nr)} cells")
    ...
    # Get donors in order: responders first, then non-responders
    responder_donors = df[df['response'] == 'responder']['donor_id'].unique().tolist()
    non_responder_donors = df[df['response'] == 'non-responder']['donor_id'].unique().tolist()
    all_donors = responder_donors + non_responder_donors

    if verbose:
        print(f"\nTotal: {len(all_donors)} donors, {len(df)} cells")

    # Define channels
    channels = [
        (['actin_mean'], 'Actin', 'green'),
        (['cd4_mean'], 'CD4', 'blue'),
        (['cd45ra_mean'], 'CD45RA', 'purple'),
    ...
    fig, axes = plt.subplots(n_donors, n_channels, figsize=(3*n_channels, 2.5*n_donors))
    ...
    for row, donor in enumerate(all_donors):
        donor_df = df[df['donor_id'] == donor]
        response = donor_df['response'].iloc[0]
        bg_color = bg_responder if response == 'responder' else bg_non_responder
        row_info.append((row, bg_color))

        # Short donor label (just the ID number)
        short_label = donor.split()[-1] if len(donor.split()) > 2 else donor

        for col, (col_names, channel_name, color) in enumerate(channels):
            ax = axes[row, col]

            col_name = next((c for c in col_names if c in donor_df.columns), None)

            if col_name and col_name in donor_df.columns:
                data = donor_df[col_name].dropna()
                if len(data) > 0:
    ...
    plt.suptitle(f'Responder vs Non-Responder Comparison\n({len(responder_donors)} responders, {len(non_responder_donors)} non-responders)',
...
def plot_combined_comparison(base_dir, verbose=True):
    ...
    plt.suptitle('Responder vs Non-Responder (All Donors Combined)',
                 fontsize=14, fontweight='bold')
