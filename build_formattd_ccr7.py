#!/usr/bin/env python3
"""Build per-patient formatted CCR7 image folders from clean cell list.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Collect clean cells from padded CCR7 folders into per-patient formatted folders."
    )
    parser.add_argument(
        "--root",
        type=Path,
    ...
    parser.add_argument(
        "--source-dirname",
        default="padded_ccr7",
        help="Source folder name under each image directory (default: padded_ccr7)",
    )
    parser.add_argument(
        "--output-dirname",
        default="formattd_ccr7",
        help="Output folder created in each patient root (default: formattd_ccr7)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    for pid in target_patient_ids:
        ...
        for patient_root in roots:
            ...
            for entry in entries:
                ...
                dst_stem = f"{Path(entry.target_name).stem}_ccr7"
