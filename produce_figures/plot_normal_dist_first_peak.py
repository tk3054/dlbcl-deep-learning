#!/usr/bin/env python3
"""
Normal Distribution Fit to First Peak

Fits a normal (Gaussian) distribution to the first peak of intensity distributions
for PacBlue (CD45RA) and PE (CCR7) channels. Uses peak detection to identify the
first mode and fits only to data near that peak.

Usage examples:
    python plot_normal_dist_first_peak.py
    python plot_normal_dist_first_peak.py --peak-window 0.3
    python plot_normal_dist_first_peak.py --channels pacblue pe
"""
...
OUTPUT_FILE = "all_samples_normal_dist_first_peak.png"
...
CHANNELS = {
    'actin': ChannelConfig("actin_mean", "Actin-FITC", "green"),
    'cd4': ChannelConfig("cd4_mean", "CD4-PerCP", "blue"),
    'pacblue': ChannelConfig("cd45ra_sparkviolet_mean", "CD45RA-PacBlue", "purple"),
    'pe': ChannelConfig("ccr7_mean", "CCR7-PE", "orange"),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fit normal distributions to first peaks in intensity distributions."
    )
    parser.add_argument(
        "--base-path",
        type=Path,
...
def plot_normal_dist_fits(
    ...
    """
    Create plots with normal distribution fits to first peaks.
    ...
    fig.suptitle(
        f"Normal Distribution Fits to First Peak (n={len(df)} cells)",
...
def main() -> None:
    ...
    print("=" * 60)
    print("NORMAL DISTRIBUTION FITS TO FIRST PEAK")
    print("=" * 60)
